...
fn prep_short(bench: &mut Bencher) {
    bench.iter(|| { queries_core::preprocess("Shut up"); })
}

fn prep_medium(bench: &mut Bencher) {
    bench.iter(|| queries_core::preprocess("Book me a uber right now"))
}

fn prep_long(bench: &mut Bencher) {
    bench.iter(|| {
        queries_core::preprocess("Book me a table for four people at a Café nearby for tomorrow night at 8pm")
    })
}

fn prep_longer(bench: &mut Bencher) {
    bench.iter(|| {
        queries_core::preprocess(
            "Book me a table for four people at a Café nearby for tomorrow night at 8pm \
