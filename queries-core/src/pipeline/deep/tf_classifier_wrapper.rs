...
impl TFClassifierWrapper<TFClassifierWrapper<Probability>> {
    pub fn new_intent_classifier(intent_config: ArcBoxedIntentConfig)
                                 -> Result<TFClassifierWrapper<Probability>> {
        let (mut tf_model, intent_model) = get_model_file(&intent_config, |pb_config| {
            pb_config.get_intent_classifier_path()
        })?;
        let classifier =
...
impl TFClassifierWrapper<TFClassifierWrapper<Array1<Prediction>>> {
    pub fn new_tokens_classifier(intent_config: ArcBoxedIntentConfig)
                                 -> Result<TFClassifierWrapper<Array1<Prediction>>> {
        let (mut tf_model, intent_model) = get_model_file(&intent_config, |pb_config| {
            pb_config.get_tokens_classifier_path()
        })?;
        let classifier: BoxedClassifier = if intent_model.has_crf {
            Box::new(TensorFlowCRFClassifier::new(&mut *tf_model,
                                                  intent_config
                                                      ...
                                                      .get_slots()
                                                      .len() as
                                                  u32, // TODO avoid get_pb_config twice
                                                  intent_model.get_input_node().to_string(),
                                                  intent_model.get_output_node().to_string(),
...
fn get_model_file<F>(intent_config: &ArcBoxedIntentConfig,
                     path_extractor: F)
                     -> Result<(Box<Read>, PBModelConfiguration)>
    where F: Fn(&PBIntentConfiguration) -> &str
{
    ...
    let tf_model = intent_config
        .get_file(path::Path::new(&model_configuration.get_model_path()))?;
...
impl ClassifierWrapper<PreprocessorResult, Probability> for TFClassifierWrapper<Probability> {
    fn run(&self, preprocessor_result: &PreprocessorResult) -> Result<Probability> {
        let feature_processor = DeepFeatureProcessor::new(self.intent_config.clone(),
                                                          &self.intent_model.get_features());
...
impl ClassifierWrapper<PreprocessorResult, Array1<Prediction>>
    for TFClassifierWrapper<Array1<Prediction>> {
...
mod test {
    ...
    fn intent_classifier_works() {
        let paths = fs::read_dir(file_path("snips-sdk-models-protobuf/tests/intent_classification/",),)
            .unwrap();
        ...
        for path in paths {
            ...
            let intent_classifier = TFClassifierWrapper::new_intent_classifier(intent_config)
                .unwrap();
    ...
    fn tokens_classifier_works() {
        let preprocessor = DeepPreprocessor::new("en").unwrap();
        let preprocessor_result = preprocessor
            .run("Book me a table for two people at Le Chalet Savoyard")
            .unwrap();
