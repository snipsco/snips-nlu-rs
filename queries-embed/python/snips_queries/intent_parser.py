...
class IntentParser(object):
    ...
    def __init__(self, data_path = None, data_binary = None):
        ...
        if data_path is not None:
            ...
            exit_code = lib.intent_parser_create_from_dir(data_path.encode("utf-8"), byref(self._parser))

        if data_binary is not None:
            self._parser = pointer(c_void_p())
            bytearray_type = c_char * len(data_binary)
            exit_code = lib.intent_parser_create_from_binary(bytearray_type.from_buffer(data_binary), len(data_binary), byref(self._parser))
    ...
    def get_intent(self, query, threshold=0.):
        pointer = c_char_p()
        lib.intent_parser_run_intent_classification(self._parser, query.encode("utf-8"),
    ...
    def get_entities(self, query, intent):
        pointer = c_char_p()
        lib.intent_parser_run_tokens_classification(self._parser, query.encode("utf-8"),
            intent.encode("utf-8"), byref(pointer))
        result = string_at(pointer)
        lib.intent_parser_destroy_string(pointer)
