...
if [ -e ../../../../target/$TARGET_BUILD_TYPE/libsnips_nlu.a ] &&
   [ $SYSTEM = macos ]; then
    echo "Using macOS local build"
    cp ../../../../target/$TARGET_BUILD_TYPE/libsnips_nlu.a $OUTDIR
    ...
    if [ ${#ARCHS_ARRAY[@]} -eq 1 ]; then
        ...
        LOCAL_LIBRARY_PATH=$PROJECT_DIR/../../../../target/$ARCHS_ARRAY-apple-ios/$TARGET_BUILD_TYPE/libsnips_nlu.a
        ...
        for arch in "${ARCHS_ARRAY[@]}"; do
            ...
            LOCAL_LIBRARY_PATH=$PROJECT_DIR/../../../../target/$arch-apple-ios/$TARGET_BUILD_TYPE/libsnips_nlu.a
            ...
            cp $LOCAL_LIBRARY_PATH $OUTDIR/libsnips_nlu_$arch.a
        done

        if [ $should_lipo = true ]; then
            echo "Lipo everything into $OUTDIR/libsnips_nlu.a"
            lipo -create `find $OUTDIR/libsnips_nlu_*.a` -output $OUTDIR/libsnips_nlu.a
            exit 0
        fi
    fi
fi

if [ ! -e $PROJECT_DIR/Dependencies/$SYSTEM/libsnips_nlu.a ] ||
